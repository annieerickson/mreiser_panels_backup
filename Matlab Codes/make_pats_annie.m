...
NUMVERTPIX      = 24;
NUMPIXAZIM      = 96;
% ARENA_DEGPERPIX = 360/NUMPIXAZIM; 
ARENA_DEGPERPIX = 360/NUMPIXAZIM; 
...
pattern.y_num = 13;
pattern.num_panels = 36;                                                    % This is the number of unique Panel IDs required.
...
Pats = zeros(NUMVERTPIX, NUMPIXAZIM, pattern.x_num, pattern.y_num);
...
% %% 1: stripe for CL mode. (from make_pats_RT3panelsHigh)
k=1;
stripe=6;
% Pats(:, (PIXFOR360/2)-(stripe/2)+1-4:(PIXFOR360/2)+(stripe/2)-4, 1, k) = repmat(zeros(1,stripe)*LVL, NUMVERTPIX, 1);
% Pats(:, 1:stripe, 1, k) = repmat(1*LVL, NUMVERTPIX, stripe);
Pats(1:NUMVERTPIX,(PIXFOR360/2):(PIXFOR360/2)+5      ,1,k) = ones(NUMVERTPIX, 6)*7;
% Pats(1:NUMVERTPIX,1:1+5,1,k) = ones(NUMVERTPIX, 6)*7;
% temp_frames = zeros(NUMVERTPIX, NUMPIXAZIM, pattern.x_num);
%trying to increment by 1 instead of decrement
% for j = 1:pattern.x_num % for close-loop stripe fixation to work, pattern.x_num needs to be an integer factor times PIXFOR360
%     %Pats(:,:,j,k) = ShiftMatrix(Pats(:,:,j-1,k),1,'r','y');
% %     Pats(:,:,j,k) = ShiftMatrix(Pats(:,:,j-1,k),1,'r','y');
%     azim_endpoint = mod(j+5, NUMPIXAZIM+1)
%     temp_frames(1:NUMVERTPIX,j:j+5      ,j) = ones(NUMVERTPIX, 6)*7;
% end
% 
% Pats(:,:,:,k) = temp_frames;
...
%% 8 2x2 stripe same pattern, differnt y position, check if written and updated file
Pats(12:13,(PIXFOR360/2):(PIXFOR360/2)+1      ,1,8) = ones(2,2)*7;
% % horizontal shift
for j = 2:pattern.x_num % for close-loop stripe fixation to work, pattern.x_num needs to be an integer factor times PIXFOR360
% %     Pats(:,:,j,1) = ShiftMatrix(Pats(:,:,j-1,1),1,'r','y');
    Pats(:,:,j,8) = ShiftMatrix(Pats(:,:,j-1,8),1,'r','y');
% %     Pats(:,:,j,3) = ShiftMatrix(Pats(:,:,j-1,3),1,'r','y');
% %     Pats(:,:,j,4) = ShiftMatrix(Pats(:,:,j-1,4),1,'r','y');
end
...
panel_map = [5 9 2 6 10 3 7 11 4 8 12 1; 17 21 14 18 22 15 19 23 16 20 24 13; 29 33 26 30 34 27 31 35 28 32 36 25];
%testing to see if fix backward strip problem 
panel_map = flip(panel_map, 2);
